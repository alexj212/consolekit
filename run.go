...
var runScriptCmdFunc = func(cmd *cobra.Command, args []string) {
	for i, arg := range args[1:] {
		defaults.Set(fmt.Sprintf("@arg%d", i), arg)
	}
	...
	doExec := func() {
		...
		for _, cmdLine := range cmds {
			...
			cmdLine = ReplaceDefaults(nil, cmdLine)

			cmd.Printf("doExec: %s\n", cmdLine)
			res, err := cli.ExecuteCommands(cmdLine)
			cmd.Printf("%s\n", res)
			if err != nil {
...
func LoadScript(filename string) ([]string, error) {
	...
	if strings.HasPrefix(filename, "@") {
		...
		content, err := cli.scripts.ReadFile(filename[1:])
		if err != nil {
